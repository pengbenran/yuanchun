		...
		<blockquote v-else>
			<div class="total">
				...
				<div class="businessCard">
					<Calendar :months="months" :value="value" @next="next" @prev="prev" :events="events" lunar @select="select" ref="calendar" @selectMonth="selectMonth" @selectYear="selectYear" />
				</div>
		...
		methods: {
			...
			get_Time() {
				...
				if(that.userInfo.defaultLv == 1) {
					wx.showModal({
						title: '提示',
						content: '您还不是合伙人哦~无法签到，需购买合伙人大礼包',
						confirmText: '去看看',
						success(res) {
							if(res.confirm) {
								let goodsId = 69
